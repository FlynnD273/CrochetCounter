...
var smooth_index: float = 0:
	set(val):
		...
		var center := rect.size.x / 2
		var offset := smooth_index - index + 0.5
		...
		curr_stitch_label.position.x = (
			center - offset * stitch_spacing * (curr_stitch_label.scale.x + 1) / 2
		)
		prev_stitch_label.position.x = (
			center - (offset + 1) * stitch_spacing * (prev_stitch_label.scale.x + 1) / 2
		)
		next_stitch_label.position.x = (
			center + stitch_spacing - offset * stitch_spacing * (next_stitch_label.scale.x + 1) / 2
		)
...
var stitches := BaseStitch.new():
	set(val):
		stitches = val
		smooth_index = min(index, stitches.length)
